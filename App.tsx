...
function JourneyLandscapeBackground({ width, height, stage, progress, reduceMotion }: { width: number; height: number; stage: JourneyLandscapeStage; progress: number; reduceMotion: boolean }) {
  const palette = journeyLandscapePalettes[stage];
  const stageIndex = journeyLandscapeStageIndex[stage];
  return <View pointerEvents="none" style={[journeyLandscapeS.background, { width, height }]}><LinearGradient colors={palette.sky} start={{ x: 0.12, y: 0 }} end={{ x: 0.88, y: 1 }} style={StyleSheet.absoluteFill} /><View style={[journeyLandscapeS.atmosphere, { backgroundColor: palette.glow, opacity: 0.05 + Math.min(0.09, progress / 1200) }]} />{journeyBiomes.map((biome) => <JourneyBiomeSection key={biome.id} biome={biome} width={width} height={height} />)}<JourneyAmbientLife reduceMotion={reduceMotion} biomes={journeyBiomes} />{stageIndex >= 3 && <LinearGradient colors={['rgba(255,228,165,0)', 'rgba(255,228,165,0.18)', 'rgba(255,228,165,0)']} start={{ x: 0.5, y: 0 }} end={{ x: 0.5, y: 1 }} style={[journeyLandscapeS.summitLight, { top: height * 0.82, height: height * 0.18 }]} />}</View>;
}
...
function Journey({ projects, activeProject, onSelectProject, onUpdateProject, onPage, onBack, onOpenBookStudio }: { projects: Project[]; activeProject: string; onSelectProject: (title: string) => void; onUpdateProject: (title: string, changes: Partial<Project>) => void; onPage: (page: Page) => void; onBack: () => void; onOpenBookStudio: (title: string, section: StudioSection) => void }) {
  ...
  return <>
    ...
    <View style={[s.journeyMap, { height: journeyLayout.height, width: journeyMapWidth }]}>
      <JourneyLandscapeBackground width={journeyMapWidth} height={journeyLayout.height} stage={landscapeStage} progress={snapshot.progressPercent} reduceMotion={reduceMotion} />
...
function Navigation({ page, onPage, projects, activeProject }: { page: Page; onPage: (page: Page) => void; projects: Project[]; activeProject: string }) {
  ...
  const renderNavItem = (item: Page, index: number, creation = false) => {
    ...
    return <Pressable key={item} onPress={() => onPage(item)} style={s.navItem} accessibilityRole="button" accessibilityState={{ selected: active }}>
