...
function ProjectDetail({ item, userId, onClose, onReact, onBlock, onReport, onFeedbackChanged, onOpenProfile }: { item: CommunityItem | null; userId: string | null; onClose: () => void; onReact: (item: CommunityItem, reaction: ReactionType) => void; onBlock: (item: CommunityItem) => void; onReport: (item: CommunityItem) => void; onFeedbackChanged?: () => void; onOpenProfile: (item: CommunityItem) => void }) {
...
export default function Community({ userId, activeProject, projects = [], onSelectProject, initialFeedbackProjectTitle, onFeedbackOpened }: CommunityProps) {
  ...
  const [writerProfileUserId, setWriterProfileUserId] = useState<string | null>(null);
  const [writerProfileFallback, setWriterProfileFallback] = useState<CommunityWriterFallback | null>(null);
  ...
  const openWriterProfile = (item: CommunityItem) => {
    setWriterProfileUserId(item.userId);
    setWriterProfileFallback({
      userId: item.userId,
      ...
      demo: item.demo,
      book: item.feedbackMode || item.hubMode ? undefined : {
        id: item.id,
...
Object.assign(s, {
  writerNameLink: { textDecorationLine: 'underline', textDecorationColor: '#BDB7EA' },
