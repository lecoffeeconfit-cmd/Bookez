...
const FeedbackTextInput = DictationInput;
...
export function FeedbackRequestDetail({ request, userId, responseCount, onClose, onChanged, standalone = false }: { request: FeedbackRequest; userId: string | null; responseCount: number; onClose: () => void; onChanged: () => void; standalone?: boolean }) {
  ...
  const [listenerAutoPlay, setListenerAutoPlay] = useState(false);
  ...
  const load = async () => {
    ...
    const [replyResult, readerResponseResult] = await Promise.all([
      ...
      request.public_comments_enabled ? supabase.from('community_feedback_reader_responses').select('*').eq('request_id', request.id).order('created_at', { ascending: false }) : Promise.resolve({ data: [] }),
    ]);
...
function SharedManuscriptPlayer({ request, content, userId, autoPlay = false, onClose }: { request: FeedbackRequest; content: ContentItem[]; userId: string | null; autoPlay?: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    let mounted = true;
    void Promise.all([Speech.getAvailableVoicesAsync(), loadBookezSpeechVoice()]).then(([voices, savedVoice]) => {
      if (!mounted) return;
      const matchingVoice = savedVoice ? voices.find((candidate) => candidate.identifier === savedVoice.identifier) : undefined;
      setVoice(matchingVoice?.identifier);
    }).catch(() => undefined);
  ...
  const closePlayer = () => { void Speech.stop(); onClose(); };
  return <Sheet onClose={closePlayer} wide><View style={s.readerHeader}><View><Text style={s.kicker}>LISTEN TO SHARED CONTENT</Text><Text style={s.readerTitle}>{request.project_title}</Text></View><Pressable onPress={closePlayer}><Text style={s.close}>×</Text></Pressable></View><View style={s.playerCard}><Text style={s.playerIcon}>◖</Text><Text style={s.playerSection}>{item.title}</Text><Text style={s.playerMeta}>{minutesFor(wordsIn(item.text), 160)} minutes · section {index + 1} of {content.length}</Text><View style={s.playerProgress}><View style={[s.readerProgressFill, { width: `${((index + 1) / content.length) * 100}%` }]} /></View><View style={s.playerActions}><Pressable onPress={() => stopAnd(index - 1)} style={s.toolButton}><Text style={s.toolText}>‹</Text></Pressable><Pressable onPress={() => setPlaying((value) => !value)} style={s.playButton}><Text style={s.playText}>{playing ? 'Ⅱ' : '▶'}</Text></Pressable><Pressable onPress={() => stopAnd(index + 1)} style={s.toolButton}><Text style={s.toolText}>›</Text></Pressable></View></View><Text style={s.sectionLabel}>PLAYBACK SPEED</Text><View style={s.focusGrid}>{[0.75, 1, 1.25, 1.5, 2].map((value) => <Pressable key={value} onPress={() => setSpeed(value)} style={[s.pill, speed === value && s.pillActive]} accessibilityRole="button" accessibilityState={{ selected: speed === value }} accessibilityLabel={`Set playback speed to ${value} times`}><Text style={[s.pillText, speed === value && s.pillTextActive]}>{value}×</Text></Pressable>)}</View><Text style={s.hint}>Playback uses on-device text-to-speech. Change speed whenever you need to move more quickly through this writing.</Text><Pressable onPress={closePlayer} style={s.primary}><Text style={s.primaryText}>Done listening</Text></Pressable></Sheet>;
}
...
function FeedbackComposer({ request, userId, onClose, onSubmitted }: { request: FeedbackRequest; userId: string; onClose: () => void; onSubmitted: () => void }) {
  ...
  useEffect(() => {
    void supabase.from('community_feedback_responses').select('*').eq('request_id', request.id).eq('responder_id', userId).maybeSingle().then(async ({ data }) => {
      const response = data as unknown as FeedbackResponse | null;
      if (!response) return;
      setOverall(response.overall_impression ?? '');
      setStrengths(response.strengths ?? '');
      setUnclear(response.unclear_sections ?? '');
      setSuggestions(response.suggestions ?? '');
      setAdditional(response.additional_comments ?? '');
      setAnonymous(response.anonymous);
      setQuickReactions(asStringArray(response.quick_reactions));
      const answers = response.question_answers && typeof response.question_answers === 'object' && !Array.isArray(response.question_answers) ? response.question_answers as Record<string, unknown> : {};
      setQuestionAnswer(request.custom_question ? String(answers[request.custom_question] ?? '') : '');
      if (request.public_comments_enabled) {
        const { data: publicResponse } = await supabase.from('community_feedback_reader_responses').select('body,anonymous').eq('request_id', request.id).eq('responder_id', userId).maybeSingle();
        setReaderResponse(publicResponse?.body ?? '');
      }
    });
  }, [request.custom_question, request.id, request.public_comments_enabled, userId]);
  ...
  const saveNow = async (status: 'draft' | 'submitted') => {
    ...
    if (request.public_comments_enabled && savedResponse?.id && readerResponse.trim() && status === 'submitted') {
      const { error: publicResponseError } = await supabase.from('community_feedback_reader_responses').upsert({ request_id: request.id, response_id: savedResponse.id, responder_id: userId, anonymous, body: readerResponse.trim() }, { onConflict: 'request_id,responder_id' });
      if (publicResponseError) { Alert.alert('Feedback saved, but public comment could not be shared', publicResponseError.message); return; }
    }
    if (request.public_comments_enabled && savedResponse?.id && !readerResponse.trim()) await supabase.from('community_feedback_reader_responses').delete().eq('request_id', request.id).eq('responder_id', userId);
    if (status === 'submitted') { Alert.alert('Feedback submitted', 'Your detailed feedback is private between you and the author.', [{ text: 'Done', onPress: onSubmitted }]); } else Alert.alert('Draft saved', 'You can return to finish this response later.');
  };
...
Object.assign(s, {
  audiobookButton: { minHeight: 55, marginTop: 13, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 15, backgroundColor: '#F0EDFF', borderWidth: 1, borderColor: '#DDD8FA', flexDirection: 'row', alignItems: 'center' },
  audiobookIcon: { color: C.purple, fontSize: 20, width: 24, textAlign: 'center' },
  audiobookCopy: { flex: 1, marginLeft: 8 },
